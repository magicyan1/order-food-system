<template>
    <div class="vip">
        ...
        <div class="search">
            <div>
                <el-input v-model="userName" class="w-50 m-2" placeholder="请输入账号" />
            </div>
            <div>
                <el-input v-model="roleName" class="w-50 m-2" placeholder="请输入角色" />
            </div>
        </div>
            ...
            <el-table :data="Employees" style="width: 100%">
                ...
                <el-table-column label="密码">
                    <template #default="scope">
                        <span>{{ scope.row.pwd === null ? '暂无密码' : scope.row.pwd }}</span>
                    </template>
                </el-table-column>
        ...
        <el-dialog v-model="dialogVisible" title="新增员工" width="30%">
            <div class="dialog">
                <el-input v-model="addData.userName" placeholder="输入账号">
                    <template #prepend>账号</template>
                </el-input>
                <el-input v-model="addData.pwd" placeholder="输入密码">
                    <template #prepend>密码</template>
                </el-input>
                <el-input v-model="addData.phone" placeholder="输入手机">
                    <template #prepend>手机</template>
                </el-input>
                <el-input v-model="addData.roleName" placeholder="输入手机">
        ...
        <el-dialog v-model="editDialogVisible" title="编辑员工" width="30%">
            <div class="dialog">
                <el-input v-model="editData.userName" placeholder="输入账号">
                    <template #prepend>账号</template>
                </el-input>
                <el-input v-model="editData.pwd" placeholder="输入密码">
                    <template #prepend>密码</template>
                </el-input>
                <el-input v-model="editData.phone" placeholder="输入手机">
                    <template #prepend>手机</template>
                </el-input>
                <el-input v-model="editData.roleName" placeholder="输入手机">
                    <template #prepend>身份</template>
                </el-input>
            </div>
            <template #footer>
                <span class="dialog-footer">
...
import { getEmployees, editEmployees, addEmployees, delEmployees, searchEmployees } from "../../apis/system/employees";
